...
def render() -> None:
    ...
    try:
        ...
        beef_val = float(emissions[emissions["food"] == "Beef (beef herd)"]["ghg_kg"].iloc[0])
        lentil_val = float(emissions[emissions["food"] == "Lentils"]["ghg_kg"].iloc[0])
        cards.insight_callout(
            f"One kilogram of beef produces {beef_val/lentil_val:.0f}x more greenhouse gases "
            f"than one kilogram of lentils ({beef_val:.0f} kg CO2 vs {lentil_val:.1f} kg)."
        )
    ...
    st.markdown('<p style="color:#574B42;font-size:0.9rem;">Countries where plants dominate the plate tend to live longer.</p>', unsafe_allow_html=True)
    ...
    try:
        ...
        if not merged.empty and len(merged) > 10:
            fig2 = go.Figure(go.Scatter(
                x=merged["meat_pct"].astype(float),
                y=merged["life_expectancy"].astype(float),
                mode="markers",
                marker=dict(
                    size=8,
                    color=merged["meat_pct"].astype(float),
                    colorscale=[[0, "#1F6F5C"], [0.5, "#E8A317"], [1, "#C0392B"]],
                    showscale=True,
                    colorbar=dict(title="Meat %"),
                ),
                text=merged["name"],
                hovertemplate="%{text}<br>Meat: %{x:.0f}%<br>Life: %{y:.0f} yrs<extra></extra>",
            ))
            fig2.update_layout(
